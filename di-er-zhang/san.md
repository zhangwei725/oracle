...
3. 示例代码
   ...
   6、查询所有员工中ename 包含数字S,或者入职日期中包含数字1或者工资中含有数字5

   ```
   SELECT * 
   FROM emp 
   WHERE ename LIKE '%SCO%' OR hiredate LIKE '%1%' OR sal LIKE '%5%'
   ```
...
2. 位置：紧跟在FROM子句后
3. 内容：由一至多个限定条件组成，限定条件由表达式, 比较符, 字面值组成所有字符串和日期要用单引号括起来，数值不需要单引号。
4. ,能过滤大部分数据库写在语句,Oracle底层自动会优化对我们的语句,